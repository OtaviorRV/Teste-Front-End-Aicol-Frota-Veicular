    ...
    <ng-template #actionsTemplate let-v>
      <div class="cell-actions">
        ...
        <button
          type="button"
          class="btn icon"
          [title]="canDeleteVehicle(v).reason ?? ''"
          [attr.aria-disabled]="!canDeleteVehicle(v).allowed ? 'true' : null"
          [style.opacity]="!canDeleteVehicle(v).allowed ? '0.4' : null"
          [style.cursor]="!canDeleteVehicle(v).allowed ? 'not-allowed' : null"
          (click)="delete(v)"
        >
          <svg xmlns="http://www.w3.org/2000/svg" style="width:14px;height:14px" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
